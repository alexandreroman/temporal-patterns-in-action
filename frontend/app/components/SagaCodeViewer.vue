...
const SOURCES: Record<Lang, SagaSource> = {
  go: {
    ...
    lines: [
      ...
      "        func(c workflow.Context) error {",
      "            return workflow.ExecuteActivity(c,",
      "                a.RefundPayment, txID, txnID, input.Amount,",
      "            ).Get(c, nil)",
      "        },",
      "    )",
      "",
  ...
  java: {
    ...
    lines: [
      ...
      "",
      "        // Step 2 — charge payment",
      "        String txnId = activities.chargePayment(txId, input, itemId);",
      "        saga.addCompensation(",
      "            () -> activities.refundPayment(txId, txnId, input.amount()));",
      "",
      "        // Step 3 — ship the order",
  ...
  python: {
    ...
    lines: [
      ...
      "            compensations.append(lambda:",
      "                workflow.execute_activity(",
      "                    refund_payment, tx_id, txn_id, input.amount,",
      "                    start_to_close_timeout=timedelta(seconds=6)))",
      "",
      "            # Step 3 — ship the order",
