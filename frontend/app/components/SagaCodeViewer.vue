...
const SOURCES: Record<CodeLang, SagaSource> = {
  ...
  typescript: {
    ...
    lines: [
      ...
      "    try {",
      "        // Step 1 — reserve inventory",
      "        const itemId = await a.reserveInventory(txID, input);",
      "        compensations.push(() => a.releaseInventory(txID, itemId));",
      "",
      "        // Step 2 — charge payment",
      "        const paymentId = await a.chargePayment(txID, input, itemId);",
      "        compensations.push(() => a.refundPayment(txID, paymentId, input.amount));",
      "",
      "        // Step 3 — ship the order",
      "        const trackingId = await a.shipOrder(txID, input);",
      "        compensations.push(() => a.cancelShipment(txID, trackingId));",
      "",
      "        // Step 4 — send confirmation",
    ...
    stepLines: {
      "reserve-inventory": [13, 15],
      "charge-payment": [17, 19],
      "ship-order": [21, 23],
      "send-confirmation": [25, 26],
    },
    compLines: {
      "release-inventory": [15, 15],
      "refund-payment": [19, 19],
      "cancel-shipment": [23, 23],
    },
  },
  python: {
    label: "Python",
